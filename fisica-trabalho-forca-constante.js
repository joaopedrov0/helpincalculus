function trabalhoForcaConstante(){
    ...
    switch(''){
        case newton:
            ...
            resolution.innerHTML = `Como você quer descobrir o valor da força, vamos isolar ela na fórmula <br>F = (Δr . cos α)/τ <br> F = (${desloc} . ${angulo}) / ${trabalho} <br>Agora é só substituir os valores e fazer a conta de multiplicação!`
            result.innerHTML = `Resultado: ${res}N`
            break
        case desloc:
            //dr = (f cosa) / j
            res = trabalho / (newton * angulo)
            resolution.innerHTML = `Como você quer descobrir o valor do deslocamento, vamos isolar ele na fórmula:<br> Δr = (F . cos α) / τ <br> Δr = (${newton} . ${angulo}) / ${trabalho}<br>Agora é só substituir os valores e fazer a conta de multiplicação!`
            result.innerHTML = `Resultado: ${res}m`
            break
        case angulo:
            //cosa = (f dr) / j
            res = trabalho / (newton * desloc)
            resolution.innerHTML = `Como você quer descobrir o valor do ângulo, vamos isolar ele na fórmula:<br> cos α = (F . Δr) / τ <br> cos α = (${newton} . ${desloc}) / ${trabalho}<br>Agora é só substituir os valores e fazer a conta de multiplicação!`
